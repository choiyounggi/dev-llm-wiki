---
id: backend-orm-transaction-boundaries
domain: backend
category: orm
applies_to: [general]
confidence: verified
...
related: [backend-jobs-idempotent-handlers, backend-errors-exception-handling, databases-transactions-isolation-level-selection]
---
...
| External effect announcing the writes (email, webhook, event) | After commit — record it in a transactional outbox inside the transaction, deliver from a job → [backend-jobs-idempotent-handlers] |
...
| `UnexpectedRollbackException` at the outer commit | An inner joined method failed and marked the shared transaction rollback-only; find the swallowed inner exception rather than catching the rollback error — catch/log placement → [backend-errors-exception-handling] |
...
| Wrap a batch of thousands of rows in one transaction | Chunked transactions with per-chunk commit and idempotent resume via checkpointing → [backend-jobs-idempotent-handlers] | One giant transaction holds locks for the whole run, and a crash at row 9,999 redoes everything |
