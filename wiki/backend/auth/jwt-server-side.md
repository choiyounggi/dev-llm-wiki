---
id: backend-auth-jwt-server-side
domain: backend
category: auth
applies_to: [general]
confidence: verified
...
related: [databases-schema-design-requirements-to-tables, databases-indexing-index-selection, backend-reliability-timeouts-and-retries]
---
...
| A legitimate client retries a refresh call after a timeout and trips reuse detection | The family is revoked and the user re-authenticates — accept this as the safe default (theft is indistinguishable from a retry); give refresh calls a single attempt, no auto-retry → [backend-reliability-timeouts-and-retries] |
