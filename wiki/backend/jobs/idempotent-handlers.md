---
id: backend-jobs-idempotent-handlers
domain: backend
category: jobs
applies_to: [general]
confidence: verified
...
related: [backend-api-design-idempotency, databases-query-optimization-existence-and-count-checks]
---
...
| External side effect (email, charge, push) | Idempotency key derived deterministically from the message id, passed to the provider → [backend-api-design-idempotency] |
