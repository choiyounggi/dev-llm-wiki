---
id: backend-concurrency-distributed-locks
domain: backend
...
related: [backend-concurrency-shared-state-and-pools, backend-jobs-scheduled-job-overlap, databases-transactions-isolation-level-selection]
---
...
| Lock wraps a scheduled batch | Overlap prevention for jobs has simpler tools first ([backend-jobs-scheduled-job-overlap]) |
| Waiting for a busy lock | Prefer fail-fast + retry with backoff over blocking waits — queued waiters amplify outages ([backend-concurrency-shared-state-and-pools] bounded-queue rule) |
